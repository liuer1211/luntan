...
function showMask(){
	...
	//$('body').append(mask);
    //$("#mask").css("height",window.top.document.documentElement.clientHeight);
  //  $("#mask").css("width",window.top.document.documentElement.clientWidth);
   // $("#mask").show();
}  
//隐藏遮罩层  
function hideMask(){          
...
function docommonAjax(type,url,data,success,gridid,boolean){
	$.ajax({
        ...
        error:function(jqXHR, exception){
        	...
        	{
        		window.location.href=contextPath+"/login/error?error=3";
        	}
        	else if((jqXHR.responseText).indexOf("/login/sessionInvalid?error=2")>0)
        	{
        		window.location.href=contextPath+"/login/sessionInvalid?error=2";
        	}
        	else if((jqXHR.responseText).indexOf("error500")>0)
        	{
        		window.location.href=contextPath+"/admin/j_spring_security_logout";
        	}
            ...
            else if(jqXHR.status === 518)
        	{
        		dialog.alert({
					content : '登录已失效！请重新登录！',
					callback:function(v){
				        	if(v){
				        		window.open(contextPath+"/userLogin/dologout",'_top');
				        	}
				        }
				});
